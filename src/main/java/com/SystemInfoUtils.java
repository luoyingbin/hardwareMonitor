...
//        return sysFiles;
    ...
    public static JSONObject getProcessInfo() {
        ...
        //通过进程id获取一个进程信息
        OSProcess osProcess = systemInfo.getOperatingSystem().getProcess(processId);
        //进程id
        processInfo.put("processId",processId);
        //线程数
        processInfo.put("threadCount",osProcess.getThreadCount());
        //线程虚拟大小
        processInfo.put("virtualSize",formatByte(osProcess.getVirtualSize()));
        //线程实际使用物理内存
        processInfo.put("residentSetSize",formatByte(osProcess.getResidentSetSize()));
        //线程正常运行时间
        processInfo.put("upTime",runtimeMxBean.getUptime());
        //线程开始时间
        processInfo.put("startTime", DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS,new Date(runtimeMxBean.getStartTime())));
        return processInfo;
    }
    ...
    public static JSONObject getInfo() throws UnknownHostException {
        ...
        info.put("processInfo", getProcessInfo());
